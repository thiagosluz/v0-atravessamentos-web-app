...
export async function deleteMembersBulk(ids: string[]) {
  return safeAction({
    action: async () => {
      await ensureAdmin()
      ...
      // Buscar avatares dos membros para cleanup no storage
      ...
      if (members?.length) {
        const fileNames = members
          .filter(m => m.avatar?.includes("supabase.co") && m.avatar?.includes("/avatars/"))
          .map(m => m.avatar.split("/").pop())
          .filter(Boolean) as string[]

        if (fileNames.length) {
          await supabase.storage.from("avatars").remove(fileNames)
        }
      }
