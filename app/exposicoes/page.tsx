        ...
        <div className="grid gap-16 md:grid-cols-2 lg:grid-cols-2">
          {publishedExhibitions.length > 0 ? (
            publishedExhibitions.map((ex: any, index: number) => {
              ...
              const shapeClass = index % 2 === 0 ? "border-organic" : "border-organic-2"
              ...
              return (
                ...
                >
                  <div className={cn("relative aspect-[16/10] overflow-hidden shadow-2xl", shapeClass)}>
                    <OrganicImage
                      src={ex.cover_image || defaultImg}
                      fallbackSrc={defaultImg}
                      alt={ex.title}
                      shape={index % 2 === 0 ? "organic" : "organic-2"}
                      containerClassName="absolute inset-0 transition-transform duration-700 group-hover:scale-105"
                    />
                    {/* Overlay Poético */}
